...
# 如果有 dataset.py 就导入，如果没有（或者在云端出错）就静默忽略
try:
...
# --- 7. 评估报告页：智能回退静态展示 ---
    ...
    st.write("本系统在相同数据集上分别训练了纯 CNN、纯 LSTM 以及本课题提出的 **CNN-LSTM 混合模型**，以下为测试集上的评估结果。")
    
    @st.cache_data(show_spinner=False)
    def evaluate_all_models():
        # 如果能在本地找到庞大的 X.npy 文件，就执行真实推理
        if HAS_LOCAL_DATA and os.path.exists('X.npy'):
